    ...
    def extract_embeddings(self, prompts, batch_size, layers):
        ...
        for i in tqdm(range(0, len(prompts), batch_size)):
            ...
            for layer_idx in layers:
                ...
                '''
                -1表示只提取最后一个token的hidden state。
                然后resid_pre_cache收集这个batch所有prompt的最后一个token的hidden state。
                outputs['hidden_states'][0].shape
                torch.Size([16, 56, 4096]).16是batch size，56是token数量，4096是hidden size
                '''
            outputs = None
            torch.cuda.empty_cache()
        logger.info("stacking all results")
        ...
        pickle.dump([resid_pre_benign_embs],open("resid_pre_benign_embds.pkl", "wb"))
        print('saved')
        H = torch.stack(list(resid_pre_benign_embs.values()), dim=1)#shape是[10000,32,4096]，10000是prompt数量，32是layer数量，4096是hidden size
