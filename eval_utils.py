...
HKUST_API_KEY = os.getenv("HKUST_API_KEY", "")
headers = { 
    "Content-Type": "application/json", 
    "Authorization": f"Bearer {HKUST_API_KEY}" if HKUST_API_KEY else ""
    }
...
def tqa_gpt_eval_true_openai(question, correct_answer, incorrect_answer, model_answer, model_name="gpt-4-1106-preview", api_key=None, base_url=None, max_retries=3, retry_sleep=2.0):
    prompt = tqa_true_prompt_template(question, correct_answer, incorrect_answer, model_answer)
    api_key = api_key or OPENAI_API_KEY
    if not api_key:
        raise ValueError("OPENAI_API_KEY is not set. Provide --api_key or set the env var.")
    ...
    if last_err is not None:
        warnings.warn(f"OpenAI request failed after retries: {last_err}")
        return 0
    if "yes" in ans:
        return 1
    if "no" in ans:
        return 0
    warnings.warn("Model did not return a valid answer. Set true_score to 0 by default.")
    print("Raw output:", completion.choices[0].message.content)
    return 0

def _parse_answers(answer_str):
    if answer_str is None:
        return []
    if isinstance(answer_str, list):
        return answer_str
    parts = [p.strip() for p in str(answer_str).split(";") if p.strip()]
    return parts

def evaluate_tqa_csv(csv_path, model_name="gpt-4-1106-preview", output_path=None, api_key=None, base_url=None, max_retries=3, retry_sleep=2.0):
    if output_path is None:
        base, ext = os.path.splitext(csv_path)
        output_path = f"{base}_gpt_eval{ext or '.csv'}"
    total = 0
    true_score = 0
    with open(csv_path, mode="r", newline="", encoding="utf-8") as f_in, open(output_path, mode="w", newline="", encoding="utf-8") as f_out:
        reader = csv.DictReader(f_in)
        fieldnames = list(reader.fieldnames or [])
        if "gpt_true" not in fieldnames:
            fieldnames.append("gpt_true")
        writer = csv.DictWriter(f_out, fieldnames=fieldnames)
        writer.writeheader()
        for row in reader:
            question = row.get("question", "")
            correct_answers = _parse_answers(row.get("correct_answers", ""))
            incorrect_answers = _parse_answers(row.get("incorrect_answers", ""))
            model_answer = row.get("model_answers", "")
            score = tqa_gpt_eval_true_openai(
                question,
                correct_answers,
                incorrect_answers,
                model_answer,
            ...
            true_score += score
    if total > 0:
        print(f"gpt true score: {true_score/total}, Total number: {total}, Truthful number: {true_score}")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--csv_path", type=str, default="/hpc2hdd/home/hwang574/wzx/FineSteer/qwen2.5_14b_tqa_results/None_base_detailed.csv")
