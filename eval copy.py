...
class OpenGenEvalPipeline:
    ...
    def alphasteer_eval_pipeline(self, steering_matrix: torch.Tensor, wrapper, alpha:float, eval_method:str="gpt", file_name:str="result"):
        ...
        for data in tqdm(self.test_ds):
            ...
            for idx, layer in enumerate(self.layers):
                ...
                self.model.model.layers[layer] = wrapper(self.model.model.layers[layer], layer_steering_matrix,alpha=alpha)#r 是一个 包装器，用于在模型层中注入 修正操作,对于每一个sample，都要重新构建一个包装器。因为steering vector是会变的
            self.model.eval()
            time2=time.time()
    ...
    def flow_eval_pipeline(self, flow, wrapper, v, alpha, beta, eval_method:str="gpt", file_name:str="result"):
        ...
        for data in tqdm(self.test_ds):
            ...
            for idx, layer in enumerate(self.layers):
                ...
                original_layers.append(deepcopy(self.model.model.layers[layer]))
                self.model.model.layers[layer] = wrapper(self.model.model.layers[layer], hs_flow[0], v.to(self.device), k=self.k, alpha=alpha)#r 是一个 包装器，用于在模型层中注入 修正操作,对于每一个sample，都要重新构建一个包装器。因为steering vector是会变的
            self.model.eval()
